...
const ProductDetail = ({ product }) => {
  ...
  const rating = 5;
  return (
    <div className="flex flex-col lg:flex-row gap-7.5 py-12 px-8 lg:px-48.75 lg:pb-12 lg:pt-0 bg-gray-light-1">
      ...
      <div className="px-6 py-2.75 gap-5.5 lg:px-5.5 lg:gap-6.75 flex flex-col">
        ...
        <div className="flex flex-row">
          {Array.from({ length: totalStars }).map((_, index) => (
            <Star
              strokeWidth={0.3}
              className={index < rating ? "fill-[#F3CD03]" : "fill-bg-light"}
            />
          ))}
